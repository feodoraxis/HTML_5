...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const adaptivePostsIcons = () => {
        if ( !is_mobile() || $(window).width() > 576 || !$('.post-big').length ) {
            if ( $('.post-big').length ) {
                $('.post-big:not(.post-sm-default) .post-icon .icon').removeAttr('style');
            }
            return;
        }
        ...
        $('.post-big:not(.post-sm-default)').each(function() {
            ...
            const bottom = $(this).find('.post-title').height() + parseInt(postDate.css('margin-bottom')) + postDate.height() + 18 + 10;
